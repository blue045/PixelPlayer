...
object QueueUtils {
    ...
    /**
     * Sonic Path Shuffle: Clusters tracks based on Genre, BPM, and Mood.
     * Used primarily for offline mode to provide a more cohesive listening experience.
     */
    fun sonicPathShuffle(songs: List<Song>, random: Random = Random.Default): List<Song> {
        if (songs.size <= 2) return songs.shuffled(random)

        // Step 1: Group by clusters (Genre + Mood)
        val clusters = songs.groupBy { song ->
            "${song.genre ?: "Unknown"}-${song.mood ?: "Neutral"}"
        }

        // Step 2: Shuffle within each cluster and sort by BPM (if available)
        val processedClusters = clusters.mapValues { (_, clusterSongs) ->
            clusterSongs.shuffled(random).sortedBy { it.bpm ?: 0 }
        }

        // Step 3: Interleave clusters to avoid staying in one genre too long
        val result = mutableListOf<Song>()
        val clusterLists = processedClusters.values.map { it.toMutableList() }.toMutableList()

        while (clusterLists.isNotEmpty()) {
            val it = clusterLists.iterator()
            while (it.hasNext()) {
                val cluster = it.next()
                if (cluster.isNotEmpty()) {
                    result.add(cluster.removeAt(0))
                } else {
                    it.remove()
                }
            }
            // Shuffle cluster order for the next pass to keep interleaving varied
            clusterLists.shuffle(random)
        }
